...
I formalize an individual's day-by-day forecasting behavior as a niave Hidden Markov Model: a hidden belief-state vector that evolves according to an individually-fit and time-invariant transition function, isolation from external influence, and the observed forecasts (pulled from data) are noisy, sparse reports of those beliefs. I then propose an evaluation methodology to identify whether this model captures forecasting behavior by fitting it on subsets of each individual's forecasting timeline and measuring predictive accuracy on the witheld data. I compare performance against the collective average and a collective model fit across all forecasters. I develop an analysis methodology that allows better understanding of human psychology to be grown from both the successes and failures of this simple model.
...
Previous work by the *Good Judgement Project (GJP)* #footnote[_Superforecasting_, written by administrators of GJP, was the main inspiration behind this research project.] in collaboration with the U.S. Intelligence Community collected 888,328 geopolitical forecasts on questions such as _"Will Daniel Ortega win another term as President of Nicaragua during the late 2011 elections?"_ and identified practices, personality traits, and institutional procedures to improve systemic forecasting ability even beyond experts in the CIA, FBI, and more. @friedmanValuePrecisionProbability2018 @tetlockSuperforecastingArtScience2015 While this work explored how experts may improve, it said little about how everyday people make their predictions. In this paper, I use the dataset of geopolitical forecasts provided by the Good Judgement Project @friedmanValuePrecisionProbability2018.

Solutions approximating the initial probability distribution and transition probability matrix of Hidden Markov Models are surprisingly recent. In @tomvercauterenOnlineBayesianEstimation2005, the describe algorithms for HMMs with a discrete state space and matrix transitions without a known initial probability distribution or transition matrix: (A) An online SMC technique (Section 3), (B) a deterministic SMC estimator for smaller state-spaces (Section 4), and (C) an Approximate MAP algorithm. All of which approximate the unknown parameters and the second of which will be used later in our modeling.

Previous work by @liederBurninBiasRationality2012 attempted to model the psychological concept of anchoring -- weighting the first piece of evidence more than subsequent pieces -- by using MCMC with a limited number of iterations, and demonstrated that the perceived distribution was different from the steady-state, demonstrating a probability "burn-in" of visited states. It is this modeling that inspired the Markov-Chain formalism in this paper and which had motivated my original goal of modeling the phenomenon belief-perseverence, a close cousin of anchoring and confirmation bias. For more info, see #link(<app:original-scope>)[_the appendix section on this project's original scope_].

= The Good Judgement Project Dataset

Before introducing any forecasting model, I first describe the subset of the GJP dataset to be analyzed and show that it contains a measurable dependency structure worth modeling.
...
To guarantee the validity of my results, I filtered any IFPs labeled `voided`. And to simplify analysis and modeling by representing forecasts as a single scalar probability, later described in #link(<sec:forecasting-hmm>)[_Forecasting Hidden Markov Models_], I limited my analysis to boolean (yes/no) questions.
#footnote[Extension to $n$-choice questions can be done with vector representations, but this is unnecessary for basic analysis.]
This reduced the number of IFPs from $617$ to $382$. See @fig:ifp-timeline in the appendix for a zoomed-out understanding of the remaining questions on a timeline.



== Baseline Forecasts and User Responses

Each of the volunteers in the GJP study were assigned a unique and anonymous ID. They would make forecasts on IFPs via an online platform during each IFP's window shown in @fig:ifp-timeline. The forecasters would rate the probability of each option as a value between $0$ and $1$, with the requirement they sum to one. An example of five forecasts can be seen in @fig:survey-fcast-example.
#footnote[As with @fig:ifp-example, a number of columns were omitted, but the ones displayed are the focus of this study.]

Because users can update their forecasts on the same IFP, I decided to consider only the first forecast, which I refer to as the *baseline forecast*. Because I had filtered for only boolean IFPs, I was able to reduce the forecasts of an agent $i$ on any IFP to a single value, $Pr["answer"_i = "\"(a) yes\""]$. Unless otherwise noted, consider all forecasts below to be baseline forecasts.
#footnote[
  Only after I had done most of my project had I realized that some of the questions had important conditionals inside of the `outcomes` field instead of in the question text. E.g. "If Parti Quebecois does not hold a majority of seats in the Quebec provincial legislature beforehand :  (a) Yes, (b) No". These conditionals certainly have a large influence on the volunteers' responses which I did not consider in my project. However, because these conditionals $Pr("yes" | "condition")$ are positively correlated with $Pr("yes")$, the (hypothetical) results here are not entirely invalid.
]
...
== Evidence for Cross-IFP Dependencies
...
The top 5 pairs of which, as displayed below in @fig:corr-topk-table, coincidentally have reasonably high positive correlations, ranging from $0.266$ to $0.325$. This weakly supports using the dependency structure between an individual's forecasts.
...
= A Hidden Markov Model of Forecaster Beliefs <sec:forecasting-hmm>

Now that we have a better understanding of the data and reason to believe that forecasts are correlated across IFPs, we can go about modeling how forecasters form their beliefs and report their predictions. The first step is choosing a representation for the discrete time series of an individual's forecasts. Because we are interested in representing the internal beliefs of the human forecaster and not just representing the forecasts themselves, one promising formalization of this data is as a Hidden Markov Model (HMM) as depicted in @fig:hmm.
...
#figure(
  ...
  caption: [
    Hidden Markov Model. #footnote[The initial dot represents that the individual's background (provided in the GJP dataset) may be used to help configure their priors $vb(Belief)_0$, although this is not attempted in the paper.]
  ],
) <fig:hmm>

We assume that forecasters submit a noisy report of their hidden beliefs from a tight ($sigma = 0.05$) but unbiased ($(mu := EE[Belief_(t,ifp)]) = EE[Report_(t, ifp)]$) #footnote[When $ifp$ is forecasted at time $t$. Missing forecasts are not applicable in this statement.] normal distribution restricted to the probability range $[0, 1]$. We additionally assume that the forecasts they decide to make (or not make) at time $t$ are independent of their internal beliefs (e.g. this assumption may be invalid if individuals may be more likely to make forecasts on IFPs they have greater certainty on). Mathematically:
$
  emission(report_(t, ifp) mid(|) belief_(t, ifp)) =
  cases(
    1 & "if" report_(t, ifp) = -1,
    cal(N)_([0,1]) (report_(t, ifp) mid(|) belief_(t, ifp), sigma^2) & "otherwise",
  )
$ <eq:emission>
where $cal(N)_([0,1]) (x mid(|) mu, sigma^2)$ denotes the probability that $x$ is sampled from the $[0,1]$-restricted normal distribution with mean $mu$, variance $sigma^2$.

As mentioned in #link(<sec:related-work>)[_Related Work_], estimating the transition probabilities $trans$ and (initial belief-state) $vb(Belief)_0$ -- hereby referred to as *parameters* $vb(theta) = (vb(Belief)_0, trans)$ -- is a surprisingly recent and non-trivial ordeal.
#footnote[I may be misled, but I am learning recently that, because this belief-state model uses continuous values it is not considered a pure HMM, rather it is considered to be a *State Space Model (SSM)* which is closer to models used in control theory or modern machine learning. Because of this continuity, it's possible that if I don't enforce discrete constraints on the problem, I can optimize with gradient descent, reinforcement learning, or analytical methods. However, it is slightly late for this.]
To use the deterministic SMC-approximation method described in @tomvercauterenOnlineBayesianEstimation2005, we first need to meet a few constraints (or rewrite the algorithm completely to handle continuity and integration which is a bit beyond this project):
1. *The state space is discrete*: To ensure the state space is discrete, we make sure that the initial probability matrix is not sampled continuously, which we have already done.
2. *Transitions are represented as a stationary transition probability matrix*: We already have this.
3. *The observation space is discrete*: Our normal distribution is not discrete. Instead, we can discretize it into $b$ equal-width bins over $[0, 1]$.

To learn the transitions probabilities $trans$ and prior (initial belief-state) $vb(Belief)_0$, we can apply the deterministic SMC algorithm described in Section 4 to generate an estimation for each of the parameters. We can start a uniform prior and transition probabilities, but any Dirichlet prior over rows for the transition matrix works fine.
$
  vb(Belief)_0 <- [1 slash 2, dots.c, 1 slash 2]^TT, \ trans(vb(belief)_(t+1) mid(|) vb(belief)_t) <- [1/abs(IFPs), dots.c, 1/abs(IFPs)]
$

// Bayesian Inference for Dirichlet-Multinomials Mark Johnson Macquarie University Sydney, Australia IPAM Summer School Updated slides available from http://web.science.mq.edu.au/mjohnson/Talks.htm
// 
...
However, with only a single individual forecaster, we have only a single sample, which is extremely difficult to develop a robust model of forecasting for #footnote[If we re-added forecast updates into our dataset, we could generate a solely individual-fitted model.] -- the best fit would be to simply memorize each of the forecasts. What we can do instead is to create a model for the individual forecaster in two phases:

1. The first phase is done a single time -- fit parameters $vb(theta)$ to the collective forecast-trajectory of every user. (in the next section this is referred to as the *Collective-Model*)
2. The second phase is to then use the learned collective paramters as the initialization to fit each individual forecaster's single baseline trajectory.

= Model Evaluation and Analysis Strategy <sec:model-evaluation>
...
For any of the train/test splits below, we fit our model with the deterministic SMC algorithm only using the training data and evaluate it based on the witheld testing data.
...
3. *Time-Interval Split*: Select a start and end time such that $1 < tau_"start" < tau_"end" < mono("dataset-end-date")$. Include only data outside this interval. This is meant to contrast against the temporal cut-off split -- just how much prediction accuracy is gained by knowing forecasts in the far future? If similarly sized train/test splits have higher accuracy in this split than the cut-off split and the stationary-transition assumption appears false, then this is positive evidence for a predictably evolving belief transition function -- i.e. evidence as time progresses, the way beliefs "interact" in your head to form new ideas and beliefs mutates slowly and continuously.
...
== Full IFP Timeline Visualization

@fig:ifp-timeline was originally an interactive graph. While it may have limited usefulness in this zoomed-out form, I believe there is some utility in visualizing the general shape, such as the four-year blocks, and quantity of questions present in this dataset even after filtering.
...
== Temporal Baseline Correlation Matrix

#figure(
  image("/reports/figures/gjp/corr_matrix/corr_matrix.pdf"),
  caption: [Correlation coefficient matrix for the first 10 IFPs. Meant as a point of comparison between strongly correlated forecasts and those that may experience stronger correlations due to temporal proximity.],
) <fig:corr-matrix>
...
*AI Use*: For the report, I had used AI to generate the Hidden Markov Model diagram and #link(<app:notation-ref>)[_Notation Reference_] (both of which I had described and edited.). I had it propose better title names and minor writing improvements which I had implemented by hand.
...
*Additional Note on Motivation*: The LLM sampling mentioned in the introduction footnote is large component of my underlying motivation. The authors behind AI-2027 expressed interest in developing better geopolitical forecasting tools and have a small team working on something related. Forecasting and logically stepping through events is an extremely time consuming process and LLMs aren't great at doing this out of the box. I was originally interested in GenLM for my mini project as some way to tie LLMs to quantitative forecasts or parameters representing the modeler's assumptions. Link to a related project idea from my personal notes: https://gatlen.notion.site/automated-wargames?source=copy_link
